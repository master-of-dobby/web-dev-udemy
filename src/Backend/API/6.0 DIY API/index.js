...
app.put("/jokes/:id", (req, res) => {
  ...
  if (!joke)
    return res.status(404).send("The Joke with the given ID was not found.");
  const toUpJoke = {
    id: id,
  ...
  jokes[jokes.indexOf(joke)] = toUpJoke;
  res.json(toUpJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const toUpdate = jokes.find((joke) => joke.id === id);
  const joke = {
    id: id,
    jokeText: toUpdate.jokeText || req.body.text,
    jokeType: toUpdate.jokeType || req. body.type,
  };
  const searchIndex = jokes.find((joke) => joke.id === id);
  jokes[searchIndex] = joke;
...
app.delete("/jokes/all", (req, res) => {
  ...
  if (userKey == masterKey) {
    ...
    res.send(404).json("You are not authorized");
  }
});
